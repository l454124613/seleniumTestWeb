...
public class HttpProxyServerHandle extends ChannelInboundHandlerAdapter {
    ...
    public void channelRead(final ChannelHandlerContext ctx, final Object msg) throws Exception {
        if (msg instanceof HttpRequest) {
            ...
            if(httpProxyHook.beforeRequest(ctx,request)){
                return;
            }
    ...
    public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
        if(httpProxyHook.getMap().containsKey(ctx.channel().id())){
            httpProxyHook.getMap().remove(ctx.channel().id());
        }
    ...
    public void transData(final ChannelHandlerContext ctx, final Object msg) throws InterruptedException {
        if (cf == null) {
            Bootstrap bootstrap = new Bootstrap();
            bootstrap.group(NettyHttpProxyServer.proxyGroup) // 注册线程池
                    .channel(NioSocketChannel.class) // 使用NioSocketChannel来作为连接用的channel类
