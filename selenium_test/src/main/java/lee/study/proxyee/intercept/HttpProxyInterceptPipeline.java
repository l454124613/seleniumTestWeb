...
public class HttpProxyInterceptPipeline implements Iterable<HttpProxyIntercept> {
  ...
  public void beforeRequest(ChannelHandlerContext chc, HttpRequest httpRequest) throws Exception {
    if (this.pos1 < intercepts.size()) {
      HttpProxyIntercept intercept = intercepts.get(this.pos1++);
      intercept.beforeRequest(chc, httpRequest, this);
    }
    this.pos1 = 0;
  }

  public void beforeRequest(ChannelHandlerContext chc, HttpContent httpContent) throws Exception {
    if (this.pos2 < intercepts.size()) {
      HttpProxyIntercept intercept = intercepts.get(this.pos2++);
      intercept.beforeRequest(chc, httpContent, this);
    }
    this.pos2 = 0;
  }
