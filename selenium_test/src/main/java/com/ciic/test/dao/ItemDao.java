...
n*=jdbcTemplate.update("INSERT INTO case_version (\"chid\", \"cid\", \"status\", \"isused\", \"isnew\", \"baseid\", \"tid\") SELECT ? chid,cid,'1' status,'1' isused,'0' isnew ,baseid ,tid FROM case_version where  status!= -1 and tid=? and isused=1  GROUP BY baseid",new Object[]{lt.get(0).getValue(),id});
